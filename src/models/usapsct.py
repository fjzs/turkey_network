...
class USAPSCT(BaseModel):
    ...
    def __init__(
        ...
        self.par_fixed_link_cost = self._get_city_to_city_parameter(
            "fixed_link_cost"
        )  # f_ij
        self.par_supply = self._get_node_supply()  # s_i
        self.par_unit_cost_per_km_flow = 1  # c
        self.par_speed_trucks_kph = 60  # v
        self.par_travel_time_truck_h = self._get_travel_time(
            self.par_speed_trucks_kph
        )  # t_ij
        ...
        self.var_departure_from_hub_to_hub = (
            self._add_var_departure_from_hub_to_hub()
        )  # Dh_k
        self.var_departure_from_hub_to_dest = (
            self._add_var_departure_from_hub_to_dest()
        )  # Dc_k
    ...
    def _get_set_cities(self, max_nodes: int) -> set:
        ...
        Returns:
            set:
        """
        return set(list(self.cities_data.keys())[0:max_nodes])
    ...
    def _get_heuristic_non_hubs_nodes(self) -> set:
        ...
        if self.top_k_cities_for_hub is not None:
            supply_ids = [
                (sum(city.flow_goods_to_other_cities.values()), id)
                for id, city in self.cities_data.items()
            ]
            ...
            hubs_to_remove = [
                id for (supply, id) in hubs_to_remove if id in self.set_cities
            ]
    ...
    def _add_var_departure_from_hub_to_hub(self):
        ...
        return self.model.addVars(
            N, lb=0, ub=self.par_max_arrival_time_h, vtype=GRB.CONTINUOUS, name="Dh"
        )
    ...
    def _add_var_departure_from_hub_to_dest(self):
        ...
        N = self.set_cities
        return self.model.addVars(
            N, lb=0, ub=self.par_max_arrival_time_h, vtype=GRB.CONTINUOUS, name="Dc"
        )
    ...
    def _add_var_arrival_to_dest(self):
        ...
        N = self.set_cities
        return self.model.addVars(
            N, lb=0, ub=self.par_max_arrival_time_h, vtype=GRB.CONTINUOUS, name="A"
        )
        ...
        * Ɐ i ∈ N, Ɐ k ∈ N
        ...
        self.model.addConstrs(
            (
                self.var_flow_orig_hub[i, k]
                + gp.quicksum(
                    self.var_flow_orig_hub_hub[i, l, k]
                    for l in N
                    if (i != k) and (l != k)
                )
                == gp.quicksum(
        ...
        * Ɐ i ∈ N, Ɐ k ∈ N: i != k
        ...
        self.model.addConstrs(
            (
                ...
                >= self.par_travel_time_truck_h[i, k] * self.var_assign_orig_hub[i, k]
                for i in N
                for k in N
                if i != k
            ),
    ...
    def _add_constraint_departure_from_hub_to_destination_waits_for_customer_trucks(
        self,
    ):
        """
        ...
        * Ɐ j ∈ N, Ɐ l ∈ N
        ...
        self.model.addConstrs(
            (
                self.var_departure_from_hub_to_dest[l]
                >= self.par_travel_time_truck_h[j, l] * self.var_assign_orig_hub[j, l]
                for j in N
                for l in N
            ),
    ...
    def _add_constraint_latest_arrival_to_dest_within_limit(self):
        ...
        * Ɐ j ∈ N
        ...
        N = self.set_cities
        self.model.addConstrs(
            (self.var_arrival_to_dest[j] <= self.par_max_arrival_time_h for j in N),
    ...
    def _add_constraint_OF_location_cost(self):
        ...
        * Σ_i Z_ii * h_i
        ...
        self.model.addConstr(
            ...
            == gp.quicksum(
                self.var_assign_orig_hub[i, i] * self.par_fixed_hub_cost[i] for i in N
            ),
    ...
    def _add_var_OF_transport_collection_cost(self):
        return self.model.addVar(
            lb=0, vtype=GRB.CONTINUOUS, name="OF_transport_collection_cost"
        )
    ...
    def _add_var_OF_transport_transfer_cost(self):
        return self.model.addVar(
            lb=0, vtype=GRB.CONTINUOUS, name="OF_transport_transfer_cost"
        )
    ...
    def _add_var_OF_transport_distribution_cost(self):
        return self.model.addVar(
            lb=0, vtype=GRB.CONTINUOUS, name="OF_transport_distribution_cost"
        )
    ...
    def save_solution(self):
        ...
        solution["OF"][
            "transport_collection_cost"
        ] = self.var_OF_transport_collection_cost.X
        solution["OF"][
            "transport_transfer_cost"
        ] = self.var_OF_transport_transfer_cost.X
        solution["OF"][
            "transport_distribution_cost"
        ] = self.var_OF_transport_distribution_cost.X
        solution["OF"]["links_cost"] = self.var_OF_links_cost.X

        # Append variables
        solution["variables"] = {}
        solution["variables"]["F"] = remove_0_values(
            self.model.getAttr("X", self.var_flow_orig_hub)
        )
        solution["variables"]["Z"] = remove_0_values(
            self.model.getAttr("X", self.var_assign_orig_hub)
        )
        solution["variables"]["Y"] = remove_0_values(
            self.model.getAttr("X", self.var_flow_orig_hub_hub)
        )
        solution["variables"]["X"] = remove_0_values(
            self.model.getAttr("X", self.var_flow_orig_hub_dest)
        )
        solution["variables"]["R"] = remove_0_values(
            self.model.getAttr("X", self.var_link_hubs)
        )
        solution["variables"]["Dh"] = remove_0_values(
            self.model.getAttr("X", self.var_departure_from_hub_to_hub)
        )
        solution["variables"]["Dc"] = remove_0_values(
            self.model.getAttr("X", self.var_departure_from_hub_to_dest)
        )
        solution["variables"]["A"] = remove_0_values(
            self.model.getAttr("X", self.var_arrival_to_dest)
        )

        utils.save_dictionary(solution, file_name=self.instance_name)
